...
namespace ViewROI
{
    ...
    public class ROIRectangle2 : ROI
    {
        ...
        public override void draw(HalconDotNet.HWindow window)
        {
            ...
            if (text != null && text != "")
            {
                HTuple pars = new HTuple();
                HTuple vals = new HTuple();

                window.DispText(text, "image", midR, midC, "black", pars, vals);
            }
        }
        ...
        public override void setModelData(HTuple data)
        {
            if (data.DArr.Length >= 5)
            {
                midR = data.DArr[0];
                midC = data.DArr[1];
                phi = -data.DArr[2];//TODO: capire perchè???????????????????????????????
                length1 = data.DArr[3];
                length2 = data.DArr[4];
        ...
        //public override Control getControlType()
        //{
        //    return new UCROIRectangle2(this, STEP_MOVE);
        //}
