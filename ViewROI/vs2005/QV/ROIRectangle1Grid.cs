...
namespace ViewROI
{
    ...
    /// corner of our ROI.
    /// </summary>
    public class ROIRectangle1Grid : ROI
    {
        ...
        public ROIRectangle1Grid(int numRows, int numCols)
        {
        ...
        public override void draw(HalconDotNet.HWindow window)
        {
            ...
            {
                double deltaRow = (row2 - row1) / this.numRows;
                window.DispLine(row1 + i * deltaRow, col1, row1 + i * deltaRow, col2);
            }

            for (int i = 0; i < this.numCols; i++)
            {
                double deltaCol = (col2 - col1) / this.numCols;
                window.DispLine(row1, col1 + i * deltaCol, row2, col1 + i * deltaCol);
            }

            window.DispRectangle2(row1, col1, 0, 5 * MULT_DISEGNI, 5 * MULT_DISEGNI);
            window.DispRectangle2(row1, col2, 0, 5 * MULT_DISEGNI, 5 * MULT_DISEGNI);
            window.DispRectangle2(row2, col2, 0, 5 * MULT_DISEGNI, 5 * MULT_DISEGNI);
            window.DispRectangle2(row2, col1, 0, 5 * MULT_DISEGNI, 5 * MULT_DISEGNI);
